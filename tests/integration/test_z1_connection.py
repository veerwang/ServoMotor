...
def test_connection(port: str) -> bool:
    ...
    try:
        ...
        print("\n--- 测试 1: 读取状态字 (6041h) ---")
        try:
            status_word = modbus.read_register(Z1_SLAVE_ID, Registers.STATUS_WORD.address)
        ...
        print("\n--- 测试 2: 读取实际位置 (6064h) ---")
        try:
            pos_high = modbus.read_register(Z1_SLAVE_ID, Registers.POSITION_ACTUAL_VALUE.address)
            pos_low = modbus.read_register(Z1_SLAVE_ID, Registers.POSITION_ACTUAL_VALUE.address + 1)
        ...
        print("\n--- 测试 3: 读取编码器分辨率 (608Fh) ---")
        try:
            enc_num_h = modbus.read_register(Z1_SLAVE_ID, Registers.ENCODER_RESOLUTION_NUM.address)
            enc_num_l = modbus.read_register(Z1_SLAVE_ID, Registers.ENCODER_RESOLUTION_NUM.address + 1)
            enc_num = (enc_num_h << 16) | enc_num_l

            enc_den_h = modbus.read_register(Z1_SLAVE_ID, Registers.ENCODER_RESOLUTION_DEN.address)
            enc_den_l = modbus.read_register(Z1_SLAVE_ID, Registers.ENCODER_RESOLUTION_DEN.address + 1)
        ...
        try:
            voltage = modbus.read_register(Z1_SLAVE_ID, 0x01F7)  # 输入电压
        ...
        try:
            di_state = modbus.read_register(Z1_SLAVE_ID, Registers.DI_PHYSICAL_STATE.address)
        ...
        try:
            driver_status = modbus.read_register(Z1_SLAVE_ID, 0x01DE)
            status_names = {
                0: "Not ready",
                1: "Ready",
...
def test_initialization(port: str) -> bool:
    ...
    try:
        ...
        # 获取 Z1 配置
        config = DEFAULT_AXIS_CONFIGS[AxisName.Z1]
        ...
        # 写入回零超时
        modbus.write_register(Z1_SLAVE_ID, Registers.HOMING_TIMEOUT.address, config.homing_timeout)
        print(f"  写入回零超时: {config.homing_timeout}ms")

        # 写入堵转参数
        modbus.write_register(Z1_SLAVE_ID, Registers.BLOCKING_TORQUE.address, config.blocking_torque)
        print(f"  写入堵转扭矩: {config.blocking_torque}")

        modbus.write_register(Z1_SLAVE_ID, Registers.BLOCKING_TIME.address, config.blocking_time)
        print(f"  写入堵转时间: {config.blocking_time}ms")

        # 写入 DI 配置
        if config.di2_function is not None:
            modbus.write_register(Z1_SLAVE_ID, Registers.DI2_FUNCTION.address, config.di2_function)
            modbus.write_register(Z1_SLAVE_ID, Registers.DI2_LOGIC.address, config.di2_logic)
            print(f"  写入 DI2: 功能={config.di2_function}, 逻辑={config.di2_logic}")

        if config.di3_function is not None:
            modbus.write_register(Z1_SLAVE_ID, Registers.DI3_FUNCTION.address, config.di3_function)
            modbus.write_register(Z1_SLAVE_ID, Registers.DI3_LOGIC.address, config.di3_logic)
        ...
        # 回零超时
        homing_timeout = modbus.read_register(Z1_SLAVE_ID, Registers.HOMING_TIMEOUT.address)
        ...
        # 堵转扭矩
        blocking_torque = modbus.read_register(Z1_SLAVE_ID, Registers.BLOCKING_TORQUE.address)
        ...
        # DI2 功能
        di2_func = modbus.read_register(Z1_SLAVE_ID, Registers.DI2_FUNCTION.address)
        expected = config.di2_function
        status = "[PASS]" if di2_func == expected else "[FAIL]"
        print(f"  {status} DI2 功能: {di2_func} (期望: {expected})")

        # DI3 功能
        di3_func = modbus.read_register(Z1_SLAVE_ID, Registers.DI3_FUNCTION.address)
